<section id="hero">


    <svg class="hero-waves" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
        viewBox="0 24 150 28 " preserveAspectRatio="none">
        <defs>
            <path id="wave-path" d="M-160 44c30 0 58-18 88-18s 58 18 88 18 58-18 88-18 58 18 88 18 v44h-352z">
        </defs>
        <g class="wave1">
            <use xlink:href="#wave-path" x="50" y="3" fill="rgba(255,255,255, .1)">
        </g>
        <g class="wave2">
            <use xlink:href="#wave-path" x="50" y="0" fill="rgba(255,255,255, .2)">
        </g>
        <g class="wave3">
            <use xlink:href="#wave-path" x="50" y="9" fill="#fff">
        </g>
    </svg>

</section><!-- End Hero -->

<section id="website" class="website">
    <div class="container">

        <div class="section-title" data-aos="fade-up">
            <h2>Panduan</h2>
            <p>Panduan Pengguna Lupa Password</p>
        </div>


        <div class="row content">
            <!-- <div class="col-md-4" data-aos="fade-right">
            <img src="assets/img/details-1.png" class="img-fluid" alt="">
          </div> -->
            <div class="col-md-12" data-aos="fade-up">

                <p style="text-align:justify">
                    Perihal kendala lupa password pada saat melakukan login pada aplikasi "Komisi Etik", terdapat 4
                    tahapan yang harus dilakukan untuk mendapatkan password tersebut agar dapat login kembali, yang
                    dapat dijelaskan sebagai berikut:
                    <br>
                    <br>
                    1. Buka halaman <a href="https://etikpenelitian.upi.edu/">http://etikpenelitian.upi.edu/</a> :
                    kemudian pilih halaman login
                    <br>
                    2. Pilih tombol lupa password
                    <br>
                    3. User diminta memasukan alamat email
                    <br>
                    4. Sistem akan mengirim ulang password ke email
                </p>


            </div>
        </div>


    </div>
</section><!-- End Details Section -->